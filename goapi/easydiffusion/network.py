...
class RequestManager:
    ...
    def http(self, method, url: str, data: dict | None = None):
        ...
        if method == "POST":
            ...
            request.setHeader(
                QNetworkRequest.ContentLengthHeader, data_bytes.size())
    ...
    def _finished(self, reply: QNetworkReply): 
        print(f"RequestManager: finished", self.mcb)
        # future = None
        # try:
        #     code = reply.error()  # type: ignore (bug in PyQt5-stubs)
...
async def _try_download(network: QNetworkAccessManager, url: str, path: Path):
    ...
    if not out_file.open(QFile.ReadWrite | QFile.Append):  # type: ignore
        raise Exception(
            f"Error during download: could not open {path} for writing")

    request = QNetworkRequest(QUrl(_map_host(url)))
    request.setAttribute(QNetworkRequest.FollowRedirectsAttribute, True)
    if out_file.size() > 0:
        print(
            f"Found {path}.part, resuming download from {out_file.size()} bytes")
        request.setRawHeader(
            b"Range", f"bytes={out_file.size()}-".encode("utf-8"))
    ...
    def handle_finished():
        ...
        if reply.error() == QNetworkReply.NetworkError.NoError:  # type: ignore (bug in PyQt5-stubs)
            ...
            finished_future.set_exception(
                NetworkError(416, "Resume not supported", url))
        else:
    ...
    for retry in range(3, 0, -1):
        try:
            ...
            if e.code == 416:  # Range Not Satisfiable
                print("Download received code 416: Resume not supported, restarting")
                ...
                raise NetworkError(
                    e.code, f"Failed to download {url}: {e.message}", url)
        except Exception as e:
            raise Exception(f"Failed to download {url}: {e}") from e

        print(f"Retrying download of {url}, {retry - 1} attempts left")
